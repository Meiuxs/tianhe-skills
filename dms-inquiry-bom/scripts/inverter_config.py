...
def find_inverter_combinations(inverters: pd.DataFrame, target_power: float,
    ...
    def find_combos_with_groups(power_groups, target, tol, max_combos, check_stock=True):
        ...
        combinations.sort(key=lambda x: sum(item[3] for item in x))
        return combinations
    ...
    mixed_combos = find_combos_with_groups(all_power_groups, target_power, tolerance, max_combinations, stock_sufficient)
...
def format_combination(combo_data: dict) -> dict:
    ...
    return {
        ...
        'items': items,
        'total_price_rank': sum(item[3] for item in combo),
        'brand': brand,
            ...
            for item in result['items']:
                ...
                for _, row in inverters.iterrows():
                    if row['物料编号'] == item['code']:
                        ...
                        break
                print(f"    - {item['code']}: {item['power']}kW × {item['quantity']}台 = {item['subtotal']}kW [{item['brand']}]{stock_info}")
            print()
