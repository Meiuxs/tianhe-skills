...
from inverter_config import (  # noqa: E402
...
class TestCalculateInverterRange(unittest.TestCase):
    ...
    def test_basic_calculation(self):
        """基本计算：组件572kW，已有100kW逆变器，比例1.1~1.2"""
        ...
        # 总功率范围: 572/1.2 ~ 572/1.1 = 476.67 ~ 520.00
        # 需要新增: max(0, 476.67-100) ~ max(0, 520.00-100) = 376.67 ~ 420.00
        self.assertAlmostEqual(need_min, 476.67 - 100, places=1)
        self.assertAlmostEqual(need_max, 520.00 - 100, places=1)
        self.assertAlmostEqual(total_min, 476.67, places=1)
        self.assertAlmostEqual(total_max, 520.00, places=1)
    ...
    def test_custom_ratio(self):
        ...
        need_min, need_max, _ = calculate_inverter_range(
            component_power=100, existing_power=0,
            ratio_min=1.0, ratio_max=1.5,
        )
        self.assertAlmostEqual(need_min, 100 / 1.5)
        self.assertAlmostEqual(need_max, 100 / 1.0)
...
class TestFindInverterCombinations(unittest.TestCase):
    """测试 find_inverter_combinations（纯逻辑，Mock 数据）"""
    ...
    def test_all_items_have_stock(self):
        ...
        for combo_data in combos:
            for item in combo_data['combo']:
                code, power, qty = item[0], item[1], item[2]
    ...
    def test_no_stock_insufficient(self):
        """库存不足时找不到方案"""
        # 把所有库存设为0
    ...
    def test_sorted_by_price_rank(self):
        """按价格排序"""
        combos = find_inverter_combinations(
            self.inverters, target_power=50.0, tolerance=0.1,
        ...
        if len(combos) > 1:
            ranks = [sum(item[3] for item in c['combo']) for c in combos]
...
class TestFormatCombination(unittest.TestCase):
    ...
    def test_format_contains_total_power(self):
        """格式化结果包含总功率"""
    ...
    def test_mixed_brand_format(self):
        """混合品牌格式化"""
        combo = {
            'combo': [('AB001347', 50, 1, 5, '华为')],
            'brand': '混合',
        ...
        self.assertEqual(result['brand'], '混合')
